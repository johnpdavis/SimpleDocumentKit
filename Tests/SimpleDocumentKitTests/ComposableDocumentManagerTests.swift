...
class MockComposableDocument: ComposableDocument, ManageableMetaDataContaining, @unchecked Sendable {
...
class ComposableDocumentManagerTests: XCTestCase {
    ...
    func testComposableDocumentManagerInit() async throws {
        ...
        manager.processLocalResult(result)
        
        let localDocuments = await MainActor.run { manager.localDocuments }
        XCTAssertEqual(localDocuments.count, 2)

        let doc1 = localDocuments[0]
        await doc1.open()
        XCTAssertEqual(doc1.metaData, metaData1)

        let doc2 = localDocuments[1]
        await doc2.open()
